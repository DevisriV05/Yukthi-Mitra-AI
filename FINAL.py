...
def run_quiz_ai_split(selected_lang, selected_level, user_profile, all_profiles_data, username):
    ...
    st.caption(f"Language: {selected_lang} • Level: {selected_level}")
    ...
    if not st.session_state.get("quiz_submitted"):
        ...
        with colC:
            change = st.button("🧭 Change Language/Level")
...
def show_timetable(user_profile):
    ...
    for day in range(1, 8):
        ...
        line = ", ".join([f"{t} – {h} hr" for t,h in summary.items()])
...
def show_study_materials(language):
    st.subheader(f"📚 Study Materials for {language}")
...
def main():
    ...
    with st.sidebar:
        ...
        if st.button("📊 Dashboard", key="nav_dashboard"):
            st.session_state.active_page = "Dashboard"
        if st.button("🗓 Timetable", key="nav_timetable"):
            st.session_state.active_page = "Timetable"
        if st.button("⏳ Pace Tracker", key="nav_pace"):
            st.session_state.active_page = "Pace"
        if st.button("📚 Study Materials", key="nav_materials"):
    ...
    if username and "welcomed" not in st.session_state:
        ...
        st.success(f"✨ Hey {username}, welcome to Yukthi-Mitra — your personal tutor! ✨")
        ...
        if st.session_state.active_page == "Dashboard":
            ...
            start = st.button("▶ Start Quiz", disabled=st.session_state.quiz_started)
            if start:
                st.session_state.quiz_started = True
                st.session_state.pop("quiz_questions", None)
                st.session_state.pop("quiz_id", None)
